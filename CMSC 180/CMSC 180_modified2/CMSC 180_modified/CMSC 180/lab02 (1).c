...
void mmt(float *subX, int m, int num_cols) {
    int i, j;
    ...
    for (i = 0; i < num_cols; i++) {
        ...
        for (j = 0; j < m; j++) {
            if (denominator != 0.0f) {
                subX[i * m + j] = (subX[i * m + j] - min) / denominator;
            } else {
                subX[i * m + j] = 0.0f;
            }
        }
...
void *worker_func(void *args) {
    ...
    //print_submatrix(my_args->subX, my_args->m, my_args->num_cols, my_args->t_id);

    mmt(my_args->subX, my_args->m, my_args->num_cols);
...
int main(int argc, char *argv[]) {
    ...
    if (input_file != NULL) {
        ...
        for (int r_idx = 0; r_idx < n; r_idx++) {
            for (int c_idx = 0; c_idx < n; c_idx++) {
                if (fscanf(fp, "%f", &X[c_idx * n + r_idx]) != 1) {
    ...
    for (int i = 0; i < t; i++) {
        int assigned_c = (i == t - 1) ? (col_chunk + rem) : col_chunk;
        
        t_args[i].subX = &X[curr_c * n];
        t_args[i].m = n;
        t_args[i].num_cols = assigned_c;
        t_args[i].t_id = i + 1;
