# Lab05 Benchmark Script — Different PCs (Table 1 + Table 2)
#
# SETUP REQUIRED:
# 1. Copy lab05.exe and config_multipc.txt to each slave PC
...
#
# The slave time elapsed is reported on each slave's terminal —
# this script captures only the master time. Read the slave times
# off each slave's screen and record the max manually.
#
...
#
# On each slave PC, run:
#   ./lab05.exe <n> 5001 1 config_multipc.txt <core_id>
...
$runs = 3
$masterPort = 5000
$outputFile = "lab05_results_multipc.csv"
...
Write-Host "=========================================="
Write-Host "Lab05 — Different PCs (Multi-PC Mode)"
Write-Host "=========================================="
Write-Host ""
Write-Host "IMPORTANT: Start slave processes on remote PCs BEFORE each run."
Write-Host "This script only runs the master and captures the master time."
Write-Host "Slave compute times must be read off each slave's terminal."
Write-Host ""

"n,t,Run,Master_seconds" | Out-File -FilePath $outputFile -Encoding UTF8
...
foreach ($n in $nValues) {
    foreach ($t in $tValues) {
        for ($run = 1; $run -le $runs; $run++) {
            ...
            Write-Host "Start $t slaves on remote PCs (with their core_id), then press Enter..."
            Read-Host
...
Write-Host ""
Write-Host "=========================================="
Write-Host "Lab05 (Multi-PC) sweep complete!"
Write-Host "Master times saved to: $outputFile"
Write-Host "Record slave max times manually from each slave's terminal."
Write-Host "=========================================="
Write-Host ""

$results = Import-Csv $outputFile

Write-Host "TABLE 1 (master) — Different PCs"
Write-Host "================================="
