#!/bin/bash
# ============================================================
# Lab 05 — Table 1: Single PC, No Core Affinity
# ============================================================
# Captures BOTH the master time (LRP05 Table 1) and the
# maximum slave compute time (LRP05 Table 2) for each (n, t, run).
#
# Usage:
#   chmod +x table1.sh
#   ./table1.sh
#
# Output:
#   - lab05_table1.csv     (n, t, run, master_time, slave_max)
#   - formatted Table 1 + Table 2 printed to terminal
# ============================================================

EXE="./lab05"
MASTER_PORT=5000
CSV="lab05_table1.csv"
LOG_DIR="lab05_slavelogs_t1"
...
echo "=========================================="
echo "Lab 05 — Table 1: Single PC, No Affinity"
echo "=========================================="
echo ""
...
for n in "${N_VALUES[@]}"; do
    for t in "${T_VALUES[@]}"; do
        ...
        for ((run=1; run<=RUNS; run++)); do
            ...
            SLAVE_LOGS=()
            for ((i=1; i<=t; i++)); do
                LOG="$LOG_DIR/n${n}_t${t}_r${run}_s${i}.log"
                SLAVE_LOGS+=("$LOG")
                $EXE $n $((MASTER_PORT + i)) $i "$CONFIG" > "$LOG" 2>&1 &
                SLAVE_PIDS+=($!)
            done
...
echo ""
echo "=========================================="
echo "Table 1 (master) — single PC, no affinity"
echo "=========================================="
...
echo "=========================================="
echo "Table 2 (slave max) — single PC, no affinity"
echo "=========================================="
