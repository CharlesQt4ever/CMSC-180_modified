...
| 6.4 | Sweep records the **maximum** across slaves per run | **DONE** | `table3.sh` redirects each slave's stdout to a per-slave log, then `grep "time elapsed:"` and takes the max via awk |
...
**Status:** Table 3 is computed *after* sweeps by `compute_table3.sh`,
which reads lab01's `lab05_T_S.csv` (produced by `lab01_baseline.sh`) and
the master times from `lab05_table3.csv` (produced by `table3.sh`),
applies the four formulas, and writes `lab05_table3_metrics.csv`.
`LAB05_ANSWERS_DRAFT.md` provides the discussion template.
...
- `lab01.c` — serial baseline (T_S source for Paper Table 3)
- `table3.sh` — swarm sweep, auto-resume; writes
  `lab05_table3.csv` with `n,t,run,master_time,slave_max`
- `lab01_baseline.sh` — runs lab01 at n ∈ {4000, 8000, 16000} (3 runs each,
  auto-resume); writes `lab05_T_S.csv`
- `compute_table3.sh` — post-processor that reads the two CSVs and writes
  `lab05_table3_metrics.csv` (Paper Table 3)
